    ...
    def run(self):
        ...
        def PrintMessage(name, message=None, sticky=False):
            if(not sticky):
                ...
                simple_printer.set(custom_size=True, width=2,
                                   height=2, invert=True, smooth=True)
                simple_printer.text(f"{name}\n")
                simple_printer.set(custom_size=False, font="b",
                                   width=2, height=2, invert=False)
                simple_printer.text(message + "\n\n\n\n\n")
            else:
                sticky_printer.charcode(code='AUTO')
                sticky_printer.set(custom_size=True, width=2,
                                   height=2, invert=True, smooth=True)
                sticky_printer.text(f"{name}\n")
                sticky_printer.set(custom_size=False, font="b",
        ...
        def AddPrintsToUser(username, print_quantity):
            User = Query()
            prints_left = users.search(User.username == username)[
                0]["prints_left"]
            ...
            async def changeColor(self, ctx):
                ...
                if(checkIfGoodColor(r) and checkIfGoodColor(g) and checkIfGoodColor(b)):
                    if(ctx.author.id != 0):
                        ...
                        color_events.insert(
                            {'name': ctx.author.name, 'color': data[0] + ' ' + data[1] + ' ' + data[2], 'id': (r / (g+b * randint(1, 9)) * randint(1, 1500) / randint(1, 7) * randint(1, 5))})
                else:
